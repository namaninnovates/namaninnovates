...
TOTAL_FRAMES = 96
SWEEP_TRAIL_DEG = 110.0 # Authentic radar trail persistence
...
for frame_idx in range(TOTAL_FRAMES):
    ...
    # 1. Dormant Background: Ultra faint resting dots (Zero bright circles until beam arrives!)
    for node in day_nodes:
        ang = node["angle_deg"]
        diff = (sweep_angle_deg - ang) % 360.0
        if diff >= SWEEP_TRAIL_DEG:
            nx, ny = node["x"], node["y"]
            # Dormant nodes are barely visible resting grid points
    ...
    # Primary Rotating Radar Laser Beam Line
    ...
    # 3. Active Nodes: Circles ONLY appear when and where the radar beam sweeps over them!
    for node in day_nodes:
        ang = node["angle_deg"]
        diff = (sweep_angle_deg - ang) % 360.0
        if diff < SWEEP_TRAIL_DEG:
            ...
            if count == 0:
                ...
                ov_draw.ellipse([(nx - radius, ny - radius), (nx + radius, ny + radius)], fill=(r_c, g_c, b_c, 255))
                # Subtle halo during direct beam contact
                if decay > 0.4:
                    ov_draw.ellipse([(nx - (radius + 1.5), ny - (radius + 1.5)), (nx + (radius + 1.5), ny + (radius + 1.5))], outline=(240, 246, 252, int(130 * decay)), width=1)
            else:
                # 7+ Commits: Very high intensity and big radius (NO extra lines, pure bright circular node!)
                radius = 3.2 + 4.0 * decay
                ov_draw.ellipse([(nx - radius, ny - radius), (nx + radius, ny + radius)], fill=(255, 255, 255, 255))
                # High energy ion ring during peak contact
                if decay > 0.25:
                    ring_r = radius + 2.2 * decay
                    ov_draw.ellipse([(nx - ring_r, ny - ring_r), (nx + ring_r, ny + ring_r)], outline=(255, 255, 255, int(200 * decay)), width=1)
...
shutil.rmtree(frames_dir)
print("Synchronized Radar GIF compiled successfully at:", target_gif)
